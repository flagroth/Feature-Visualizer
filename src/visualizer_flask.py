...
def make_plot(source_points, compo=None, columns=None, method=None):  
    p = figure(tools = ["pan,wheel_zoom,box_select,lasso_select,reset"], width = 300, height = 300, title=method, sizing_mode='scale_width')
    x = 'x_' + method.lower()
    y = 'y_' + method.lower()
...
def visualize():
    ...
    if "RANDOM" in vizs:
        ...
        X_r_random = randomProjection.fit(train_x,train_y).transform(train_x)
    ...
    print("Plotting")
    if "PCA" in vizs:
        g.append(make_plot(source_points, comp_pca, train_x.columns, "PCA"))
    if "LDA" in vizs:
        g.append(make_plot(source_points, comp_lda, train_x.columns, "LDA"))
    if "RANDOM" in vizs:    
        g.append(make_plot(source_points, comp_random, train_x.columns, "RANDOM"))        
    if "TSNE" in vizs:    
        g.append(make_plot(source_points, method="TSNE"))
    if "MDS" in vizs:    
        g.append(make_plot(source_points, method="MDS"))
    
    p = gridplot(g, ncols=2, plot_width=600, plot_height=600, sizing_mode='scale_width')
